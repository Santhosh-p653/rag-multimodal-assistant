...
def reset_vector_store_singleton():
    ...
    yield
    if VectorStoreService._instance and hasattr(VectorStoreService._instance, "client"):
        try:
...
def sample_pil_image(tmp_path):
    ...
    draw.text((10, 10), "Cooling fan diagram printer rear", fill=(0, 0, 0))
    draw.line((0, 0, 300, 300), fill=(0, 0, 0), width=5)
    img.save(img_path)
    return img_path
...
def test_vision_vector_store_ingestion_and_search(sample_pil_image):
    ...
    assert vector_store.client.count("manual_images").count == 1, "Image failed to upsert into Qdrant"
    ...
    assert len(results_unfiltered) > 0, f"Unfiltered search returned empty"

    # Search with source_file filter
    results = search_similar_images("cooling fan diagram", source_file="test_vision_manual.pdf")
    assert isinstance(results, list)
    assert len(results) > 0, f"Filtered search returned empty"
...
def test_retrieve_context_with_vision_compatibility(sample_pil_image):
    ...
    vector_store.ingest_chunks([{
        ...
        "content": "To fix cooling fan Error E105 on X100, replace rear cable.",
        "source_file": "test_vision_manual.pdf",
        "product": "X100",
        "model": "X100",
        "embedding": [0.01] * 384
    ...
    chunks, conf, vision_hits = retrieve_context_with_vision("cooling fan error", source_file="test_vision_manual.pdf")
